...
static MGPUVideoTexture* import_d3d11(const MGPUExternalVideoBuffer* buf) {
    ...
    // ── Strategy: open the producer's shared NT handle on the consumer
    // (Dawn's) D3D11 device, then immediately CopyResource it into a brand
    // new Dawn-private (non-shared) texture and CPU-wait the copy via a
    // D3D11_QUERY_EVENT. The resulting Dawn-private texture is then handed
    // to Dawn via SharedTextureMemoryD3D11Texture2DDescriptor.
    //
    // Why this dance is necessary:
    //   * Producer textures from WGC / MediaFoundation are shared with
    //     SHARED_NTHANDLE only — no keyed mutex, no fence value exposed.
    //   * Without keyed mutex / fence, Dawn has no way to synchronise its
    //     compute reads against the producer's writes — even within the
    //     same physical GPU. This races and on NVIDIA results in
    //     DXGI_ERROR_DEVICE_HUNG inside the very first compute dispatch.
    //   * By copying into a Dawn-private texture under a CPU-side query
    //     wait, we serialise correctly without needing a keyed mutex.
    // Cost: one extra GPU-side CopyResource per frame (no PCIe transfer).
    ...
    // Open the producer's shared handle on Dawn's d11 device (just a ref to
    // the same physical memory; safe to do every frame).
...
// regardless of source memory layout) and store into rgba8unorm storage tex.
...
EXPORT int mgpuVideoTextureBGRAToRGBASharedOutput(MGPUVideoTexture* src,
    ...
    bgEntries[1].textureView = dst->view;
    ...
    // EndAccess transitions the resource state back to COMMON so the D3D11
    // client may read it.  (Dawn returns SharedFences here; we ignore them
    // for now and rely on the CPU-side wait below for ordering.)
    WGPUSharedTextureMemoryEndAccessState endState{};
    wgpuSharedTextureMemoryEndAccess(dst->shared_mem, dst->texture, &endState);

    // Block until the GPU finishes the dispatch so the D3D11 client can
    // safely sample the texture immediately after this call returns.
    ...
    // Drain Dawn events on the calling thread until our callback fires.
...
EXPORT int mgpuCopyBufferToSharedOutputTexture(
    ...
    // Transition the resource back to COMMON so the D3D11 client can read it.
    WGPUSharedTextureMemoryEndAccessState endState{};
    wgpuSharedTextureMemoryEndAccess(dst->shared_mem, dst->texture, &endState);

    // Block until the GPU copy is complete before returning.
    std::promise<void> done;
    auto fut = done.get_future();
    WGPUQueueWorkDoneCallbackInfo cbInfo{};
    cbInfo.mode      = WGPUCallbackMode_AllowProcessEvents;
...
EXPORT uint32_t mgpuSharedOutputTextureDebugReadFirstPixelDawn(
    ...
    while (mapFut.wait_for(std::chrono::milliseconds(0)) !=
           std::future_status::ready) {
        wgpuInstanceProcessEvents(minigpu.getInstance());
    }
